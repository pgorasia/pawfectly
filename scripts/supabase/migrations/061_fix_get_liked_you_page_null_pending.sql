-- Fix NULL-truthiness bug in get_liked_you_page cross-lane pending filter.
--
-- Previous implementation used:
--   AND NOT (cl.status = 'pending' AND ...)
-- With a LEFT JOIN, cl.status is NULL for most rows.
-- In SQL, NOT(NULL) = NULL, and WHERE ... AND NULL filters the row out.
-- Result: Liked You page returns 0 rows even when likes exist.
--
-- This migration makes the condition NULL-safe using COALESCE.
...
BEGIN
  ...
  visible AS (
    ...
    WHERE p.lifecycle_status IN ('active', 'limited')
      ...
      -- Exclude cross-lane pending from the Match-liker until resolved (NULL-safe)
      AND NOT (
        COALESCE(cl.status, '') = 'pending'
