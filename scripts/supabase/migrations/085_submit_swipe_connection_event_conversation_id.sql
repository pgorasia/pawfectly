-- ============================================================================
-- Migration: include conversation_id in submit_swipe connection_event when known
--
-- Goal:
-- - When a mutual connection is completed via submit_swipe AND a conversation
--   already exists (e.g. there was an incoming request that we auto-accept),
--   return that conversation id inside connection_event.
-- - This lets the client deep-link to the exact same chat thread UI (and show
--   existing pending/request messages) without extra fetches.
--
-- Notes:
-- - Backward-compatible: only adds a new JSON key inside connection_event.
-- - conversation_id will be null when no conversation exists yet.
-- ============================================================================
...
BEGIN
  ...
  IF p_action = 'accept' AND COALESCE(v_prev_action, '') <> 'accept' THEN
    ...
    IF v_reverse_same THEN
      ...
      -- If there is an incoming request conversation from the other user, promote it to ACTIVE.
      ...
      -- Resolve the conversation id (if any) so the client can deep-link.
      SELECT c.id
        INTO v_conversation_id
      FROM public.conversations c
      WHERE c.user_low = v_low
        AND c.user_high = v_high
        AND c.status IN ('request', 'active')
        AND EXISTS (
          SELECT 1
          FROM public.conversation_participants mep
          WHERE mep.conversation_id = c.id
  ...
  RETURN jsonb_build_object(
    ...
    'remaining_accepts',
      ...
      END,
    -- connection event (client can show celebration without extra calls)
    'connection_event',
      CASE
