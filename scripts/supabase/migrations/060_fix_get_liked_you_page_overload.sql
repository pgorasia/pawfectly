-- Fix PostgREST overload ambiguity for get_liked_you_page
--
-- The API client calls get_liked_you_page with params:
--   p_limit, p_cursor_liked_at, p_cursor_liker_id
--
-- Having an overloaded function signature that also includes p_user_id (even with a DEFAULT)
-- causes PostgREST/Supabase RPC resolution to become ambiguous (PGRST203).
--
-- This migration removes the overloaded (p_user_id, ...) version and ensures a single
-- auth.uid()-scoped function exists with the expected signature.

-- Drop the overloaded signature (created by earlier migrations) if present.
-- Signature: (p_user_id uuid, p_limit integer, p_cursor_liked_at timestamptz, p_cursor_liker_id uuid)
DROP FUNCTION IF EXISTS public.get_liked_you_page(uuid, integer, timestamptz, uuid);
...
BEGIN
  ...
  visible AS (
    ...
    LEFT JOIN public.cross_lane_connections cl
      ON cl.user_low = LEAST(v_user_id, b.liker_id)
    ...
    WHERE p.lifecycle_status IN ('active', 'limited')
      ...
      AND NOT (
        cl.status = 'pending'
        AND cl.match_user_id = v_user_id
        AND cl.pals_user_id = b.liker_id
      )

      -- Exclude anyone I've already hard-acted on in swipes (accept/reject)
      -- EXCEPT: allow chooser (pals_user) to see cross-lane pending even though they already accepted in pals lane.
      AND (
        NOT EXISTS (
          SELECT 1
  ...
  SELECT
    ...
    CASE
      WHEN v.cl_status = 'pending'
       AND v.cl_pals_user_id = v_user_id
