-- ============================================================================
-- Migration: include conversation_id in send_chat_request connection_event
--
-- IMPORTANT:
-- - This is based on the latest send_chat_request logic (as defined in 083),
--   including cross-lane pending message persistence and submit_compliment_like.
-- - The only behavioral change is adding connection_event.conversation_id for
--   SAME-LANE mutuals so the client can deep-link to the exact thread.
-- ============================================================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    RETURN jsonb_build_object(
      ...
      -- allow feed to prompt chooser immediately (pals-liker only)
      'connection_event',
        CASE
  ...
  RETURN jsonb_build_object(
    ...
    'remaining_accepts', v_like->'remaining_accepts',
    -- signal if this action completed a same-lane mutual connection
    'connection_event',
      CASE
...
GRANT EXECUTE ON FUNCTION public.send_chat_request(uuid, text, text, jsonb, uuid) TO authenticated;
