-- ============================================================================
-- Migration: Harden cross-lane pending message persistence in send_chat_request
--
-- Observed issue:
-- - In some cases, the pending cross-lane tile UI loads but the stored message is missing.
-- - Root causes can include overly-strict idempotency predicates during the UPDATE.
--
-- Fix:
-- - When a pending cross-lane row exists, write the initial message deterministically
--   if (and only if) message_body is NULL.
-- ============================================================================
